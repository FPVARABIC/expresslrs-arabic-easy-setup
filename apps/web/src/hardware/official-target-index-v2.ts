...
export class OfficialTargetIndexV2Error extends Error {
  public constructor(message: string) {
    super(message);
    this.name = "OfficialTargetIndexV2Error";
  }
}
...
export function parseOfficialTargetsV2(value: unknown): readonly OfficialTarget[] {
  if (!isRecord(value)) {
    throw new OfficialTargetIndexV2Error(
  ...
  const walk = (
    ...
    if (config !== null) {
      ...
      const radioKey =
        [...context.path]
          .reverse()
          .find((part) => roleFromPath([part]) !== null) ?? null;
