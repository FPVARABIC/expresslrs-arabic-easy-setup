...
function safeMessage(error: unknown): string {
  return (error instanceof Error
    ? error.message
    : "توقفت العملية بسبب خطأ غير معروف"
  )
    .replace(/[\u0000-\u001f\u007f-\u009f]/gu, " ")
    .replace(/[\u202a-\u202e\u2066-\u2069]/gu, "")
    .replace(/\s+/gu, " ")
...
export function ExpressLrsParityWorkbenchV2() {
  ...
  function downloadRecovery(): void {
    ...
    setStatus("تم تنزيل حزمة الاستعادة. احتفظ بها حتى اكتمال التحقق بعد الإقلاع.");
  }
  ...
  async function reconnectAndVerify(input: {
    ...
    const targetVerified =
      (match.confidence === "EXACT" && match.selected?.id === input.target.id) ||
      (input.expectedIdentity !== null &&
        samePhysicalIdentity(input.expectedIdentity, outcome.identity)) ||
      input.manualTargetWasConfirmed;
    if (!targetVerified) {
      await outcome.session.close();
      throw new Error("عاد جهاز، لكن أدلة Target لا تطابق العملية المخططة.");
    }
  ...
  async function prepareSerialTransport(input: {
    ...
    if (input.selectedMethod === "uart") {
      ...
      if (selectedTarget === null || liveIdentity.role !== selectedTarget.role) {
    ...
    <main className="parity-shell" dir="rtl">
      <header className="parity-header">
        <div>
          ...
          <p>
            مصدر رسمي، تعريف CRSF، إعدادات حقيقية، استعادة إلزامية، ونجاح
            مشروط بعودة الجهاز المتوقع.
          </p>
        </div>
        <span className={identity === null ? "parity-state" : "parity-state is-ready"}>
      ...
      <section className="parity-status" role="status" aria-live="polite">
        ...
        <span>{status}</span>
        {busy &&
        (operationAbortRef.current !== null || catalogAbortRef.current !== null) ? (
        ...
        <div className="parity-card-heading">
          <div>
            ...
            <div>
              ...
              <p>الإصدارات وTargets وطرق التحديث تأتي من مصادر ExpressLRS الرسمية.</p>
            </div>
          </div>
          <button
            type="button"
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) => {
                ...
                const nextRadio =
                  roleTargets.find(
                    (target) => target.vendorKey === nextVendor,
                  )?.radioKey ?? "";
            ...
            <dl className="segment-list">
              {prepared.segments.map((segment) => (
                <div key={`${segment.address}:${segment.name}`}>
                  <dt>
                    {segment.name} · 0x{segment.address.toString(16).toUpperCase()}
                  </dt>
                  <dd>
