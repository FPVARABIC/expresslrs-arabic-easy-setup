...
describe("physical acceptance model", () => {
  ...
  it("creates an unlocked session with every step available from the start", () => {
    ...
    expect(
      Object.values(value.results).every((result) => result.status === "NOT_RUN"),
    ).toBe(true);
  ...
  it("rejects malformed and oversized imported sessions", () => {
    ...
    expect(parsePhysicalAcceptanceJson(JSON.stringify({ schemaVersion: 1 }))).toBeNull();
    expect(parsePhysicalAcceptanceJson(`"${"x".repeat(1_000_001)}"`)).toBeNull();
  });
