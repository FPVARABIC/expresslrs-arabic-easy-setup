      ...
      return Object.freeze({
        status:
          context.packageSegmentCount > 0 && context.recoveryDownloaded
            ...
            : null,
        evidence,
        reason: "A prepared package and downloaded recovery archive are required",
      });
...
function validUsbId(value: number | null): number | null {
  return value !== null && Number.isInteger(value) && value >= 0 && value <= 0xffff
    ? value
    : null;
}
...
function isStatus(value: unknown): value is PhysicalAcceptanceStepStatus {
  return typeof value === "string" && STATUS_VALUES.has(value as PhysicalAcceptanceStepStatus);
}
...
function parseContext(value: unknown): PhysicalAcceptanceContextSnapshot | null {
  ...
  return sanitizeContext({
    ...
    flashStage:
      typeof value.flashStage === "string" ? value.flashStage : null,
    statusMessage:
...
export function parsePhysicalAcceptanceSession(
  ...
  if (!isObject(value) || value.schemaVersion !== PHYSICAL_ACCEPTANCE_SCHEMA_VERSION) {
    return null;
  }
  if (
  ...
  if (Array.isArray(value.events)) {
    for (const item of value.events.slice(-MAX_EVENTS)) {
      ...
      const stepId =
        typeof item.stepId === "string" && STEP_IDS.has(item.stepId as PhysicalAcceptanceStepId)
