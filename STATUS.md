...
- Continue Hardware-independent update safety with bounded compressed-artifact and executable/Target identity validation; no trust root, persisted security store, real artifact, or writer is admitted yet.
...
- Artifact authenticity: internal provenance coherence, exact byte hashing, bounded Synthetic Manifest/root parsing, untrusted Ed25519 verification, dual-threshold Synthetic rotation/revocation, expiry evaluation, and unpersisted rollback transitions are implemented. An admitted initial root, production clock assurance, atomic persisted root/rollback state, compressed-payload validation, and corresponding-source evidence remain unimplemented.
