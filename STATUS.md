...
- The current local M2A candidate passes formatting, ESLint with zero warnings, TypeScript, nine-package dependency boundaries, security-header checks, 101 local links across 60 Markdown files, the complete Master Plan contract, 479/479 Vitest cases across 34 files, and the production Web/Pages build. The full license and advisory gates are rerun before publishing each exact candidate. This is not Hardware evidence.
...
- Continue Hardware-independent update safety with bounded Synthetic source-inventory and notice-schema inspection plus declared build-input linkage; no trust root, persisted security store, production origin, real artifact, catalog entry, or writer is admitted yet.
...
- Artifact authenticity: internal provenance coherence, exact byte hashing, bounded Synthetic Manifest/root parsing, untrusted Ed25519 verification, dual-threshold Synthetic rotation/revocation, expiry evaluation, unpersisted rollback transitions, isolated Synthetic gzip/executable identity validation, signed dual-form catalog-candidate linkage, bounded Synthetic acquisition, and exact source/notice byte evidence are implemented. An admitted initial root, production clock assurance, atomic persisted root/rollback state, production origin/network acquisition, source/archive semantic inspection, reproducible build linkage, real executable parsers, and catalog admission remain unimplemented.
...
- Add bounded Synthetic source-inventory and notice-schema inspection, then link declared build inputs without admitting any production origin, Catalog entry, or writer.
