...
- Synthetic Firmware execution now requires a schema-v1 provenance envelope. Core rebuilds fixed own data properties before observers, validates canonical metadata, freezes the nested snapshot, and blocks Target or artifact-digest disagreement before provider execution. The result is labeled `COHERENCE_ONLY`; no Firmware bytes, trusted source, or signed manifest exists.
...
- The current local M2A candidate passes the complete quality gate: 363/363 Vitest cases across 25 files, TypeScript, ESLint with zero warnings, formatting, nine-project dependency boundaries, 65 local links across 54 Markdown files, the Master Plan contract, production security-header source/build verification, and the Web production build. The license policy passes 248 package/version records with no exception, and the high-severity advisory audit finds no known vulnerability. This is not Hardware evidence.
...
- Continue Hardware-independent update safety with a signed-manifest design and byte-level artifact hashing boundary; no trust root, real artifact, or writer is admitted yet.
...
- Artifact authenticity: internal provenance coherence is implemented, but byte hashing, signed manifests, trust-root/update/revocation policy, and corresponding-source evidence remain unimplemented.
