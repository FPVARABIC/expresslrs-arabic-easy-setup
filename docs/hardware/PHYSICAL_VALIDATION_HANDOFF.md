...
| W2 | With an ST-Link/V2 or V2-1 probe (section 3, item 12), start the write by the `stlink` method | The probe's firmware version and the target voltage are shown; the connected MCU is named and matched to the Target's `stlink.cpus` **before** any erase; only the application pages are erased and written; the SWD status byte of every command is checked (a FAULT stops the write, a WAIT is retried a bounded number of times); every block is read back; the core is released to run and the device reboots into the new image. A cancel mid-write re-locks the flash and reports per step (flash lock, debug exit, USB close) whether cleanup was confirmed | It writes to a part that does not match the Target, reports success without the read-back, accepts a SWD FAULT as success, or leaves the flash unlocked after a cancel |
